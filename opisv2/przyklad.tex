...
rozbicia zostaną rozwiązane, to rozwiązany zostaje również problem wyjściowy.

\subsection{Rozbicie problemu na podproblemy}

Rozbiciem problemu na podproblemy nazywamy wskazanie takiej reguły
wnioskowania oraz takiej listy sekwentów-przesłanek, że sekwent wyjściowy
...
Pierwsza linia określa problem wyjściowy. Druga linia wskazuje regułę, za pomocą
...
musi zostać sprawdzone formalnie. Rozbicie składa się z problemu wyjściowego \(S\),
reguły \(R\) oraz listy podproblemów \(S_1,\ldots,S_n\). Jest poprawne wtedy, gdy
zastosowanie reguły \(R\) do podproblemów pozwala otrzymać problem wyjściowy.

W implementacji reguły wnioskowania mogą być stosowane tylko do sekwentów,
...
\]
Średnik oddziela tutaj kontekst bazowy od kontekstu dodatkowego.

Tak samo przekształcany jest problem wyjściowy \(S\). Nie jest to potrzebne do
samego zastosowania reguły, ale upraszcza końcowe porównanie: wynik zastosowania
reguły do przekształconych podproblemów porównujemy z problemem wyjściowym
zapisanym w tej samej konwencji.

Procedura sprawdzania wygląda więc następująco:
\[
\begin{array}{l}
\textsc{SprawdźRozbicie}(S,R,S_1,\ldots,S_n):\\[0.4ex]
\quad \text{sprowadź } S \text{ do postaci z pustym kontekstem bazowym,}\\
\quad \text{sprowadź każde } S_i \text{ do postaci z pustym kontekstem bazowym,}\\
\quad \text{sprawdź pomocniczo, czy każde } S_i \text{ jest sekwentem tautologicznym,}\\
\quad \text{wyznacz dodatkowe argumenty potrzebne regule } R,\\
\quad \text{spróbuj zastosować } R \text{ do przekształconych podproblemów,}\\
\quad \text{jeżeli zastosowanie reguły zgłasza błąd, odrzuć rozbicie,}\\
\quad \text{jeżeli otrzymany sekwent jest różny od przekształconego } S,
      \text{ odrzuć rozbicie,}\\
\quad \text{w przeciwnym przypadku zaakceptuj rozbicie.}
\end{array}
\]

Dodatkowe argumenty są potrzebne dla tych reguł, których zastosowanie nie jest
jednoznacznie wyznaczone przez same przesłanki. Na przykład przy regule
\(\vee I_1\) trzeba wskazać prawy składnik alternatywy, a przy \(RAA\) formułę,
której dowodzimy przez sprowadzenie jej negacji do sprzeczności. W takich przypadkach
brakujące dane są odczytywane z problemu wyjściowego.

Sprawdzanie tautologiczności podproblemów jest elementem pomocniczym. Nie jest
konieczne do stwierdzenia, czy rozbicie pasuje do schematu reguły, ale pozwala
odrzucić rozbicia prowadzące do podproblemów, których system i tak nie byłby
w stanie udowodnić. Jest to uproszczenie przyjęte ze względu na prototypowy
i eksperymentalny charakter systemu oraz niewielki rozmiar rozważanych formuł.
Nie zastępuje ono generowania dowodu: zaakceptowany podproblem nadal musi mieć
formalny dowód, którego kolejne linie są sprawdzane przez parser i reguły
wnioskowania.

\subsection{Generowanie rozbić przez model}

Rozbicia są generowane przez model językowy w podobny sposób jak kolejne linie
dowodu. Model otrzymuje początek tekstu zawierający linię \texttt{Smash:} z opisem
sekwentu, który ma zostać rozbity, a następnie próbuje dopisać dalszą część opisu:
wybraną regułę oraz listę podproblemów. Wygenerowany kandydat jest następnie
sprawdzany przez procedurę weryfikującą rozbicie.

Schemat generowania pojedynczego poprawnego rozbicia wygląda następująco:
\[
\begin{array}{l}
\textsc{WygenerujRozbicie}(S):\\[0.4ex]
\quad \text{utwórz prompt zaczynający się od } \texttt{Smash: } S,\\
\quad \text{wygeneruj dalsze linie opisu rozbicia,}\\
\quad \text{sprawdź, czy tekst ma pełny format rozbicia,}\\
\quad \text{sprawdź formalną poprawność rozbicia,}\\
\quad \text{jeżeli rozbicie jest poprawne, zwróć je,}\\
\quad \text{w przeciwnym przypadku ponów próbę generacji.}
\end{array}
\]

W praktyce dla jednego problemu można wygenerować kilka rozbić. Część z nich może
się powtarzać, ponieważ model często wskazuje te same najbardziej naturalne reguły.
Powtórzenia nie są całkowicie bezużyteczne: można je traktować jako informację
o tym, które rozbicia model wybiera najchętniej. Dlatego częstości występowania
poszczególnych rozbić mogą zostać użyte jako wagi przy późniejszym wyborze gałęzi
drzewa poszukiwania.

\subsection{Dwa typy wierzchołków drzewa}

Aby połączyć dekompozycję celu z generowaniem dowodów częściowych, system buduje
drzewo poszukiwania. W drzewie tym występują dwa rodzaje wierzchołków.

Pierwszy rodzaj wierzchołków reprezentuje problemy dowodowe, czyli sekwenty do
udowodnienia. Będziemy je nazywać wierzchołkami typu \(LP\), ponieważ odpowiadają
obiektom reprezentującym pojedynczy sekwent. Dzieciami takiego wierzchołka są
możliwe rozbicia danego sekwentu.

Drugi rodzaj wierzchołków reprezentuje konkretne rozbicia problemu na podproblemy.
Będziemy je nazywać wierzchołkami typu \(SP\). Dzieciami wierzchołka typu \(SP\)
są sekwenty-przesłanki występujące w danym rozbiciu.

Struktura drzewa jest więc naprzemienna:
\[
\small
\begin{array}{c}
\text{sekwent}\\[-0.2ex]
...
\;\longrightarrow\;
\begin{array}{c}
\text{podsekwenty}\\[-0.2ex]
LP_1,\ldots,LP_n
\end{array}.
\]

Wierzchołek typu \(LP\) jest rozwiązany, jeżeli udało się bezpośrednio wygenerować
dowód odpowiadającego mu sekwentu albo jeżeli przynajmniej jedno z jego rozbić
prowadzi do rozwiązanych podproblemów. Wierzchołek typu \(SP\) jest rozwiązany,
jeżeli rozwiązane są wszystkie podproblemy występujące w danym rozbiciu.

\subsection{Wartość wierzchołka}

Każdemu wierzchołkowi drzewa przypisywana jest wartość logiczna informująca, czy
problem zakodowany w tym wierzchołku został już rozwiązany. Wartość ta jest
obliczana inaczej dla obu typów wierzchołków.

Dla wierzchołka typu \(LP\), który reprezentuje pojedynczy sekwent, wystarczy znaleźć
jeden sposób jego udowodnienia. Może to być dowód wygenerowany bezpośrednio albo
jedno z poprawnych rozbić, którego wszystkie podproblemy zostały rozwiązane.
Dlatego wartość wewnętrznego wierzchołka typu \(LP\) jest alternatywą wartości jego
dzieci:
\[
    Value(LP) = \bigvee_{C \in Children(LP)} Value(C).
\]

Dla wierzchołka typu \(SP\), który reprezentuje konkretne rozbicie, sytuacja jest
inna. Rozbicie jest użyteczne dopiero wtedy, gdy uda się rozwiązać wszystkie jego
podproblemy. Dlatego wartość wewnętrznego wierzchołka typu \(SP\) jest koniunkcją
wartości jego dzieci:
\[
    Value(SP) = \bigwedge_{C \in Children(SP)} Value(C).
\]

Takie wartościowanie odpowiada naturalnej interpretacji drzewa poszukiwania.
Wierzchołek problemu wymaga znalezienia przynajmniej jednej skutecznej metody
dowodu. Wierzchołek rozbicia wymaga natomiast rozwiązania wszystkich problemów,
na które dany problem został rozłożony.

\subsection{Rozwijanie liścia}

Liść typu \(LP\) reprezentuje sekwent, dla którego system nie zna jeszcze ani
bezpośredniego dowodu, ani żadnego rozbicia na podproblemy. Rozwinięcie takiego
liścia polega na podjęciu dwóch kolejnych prób.

Najpierw system próbuje znaleźć bezpośredni dowód sekwentu za pomocą wcześniejszej
strategii przeszukiwania wielu częściowych dowodów. Jeżeli taka próba zakończy się
sukcesem, wierzchołek zostaje oznaczony jako rozwiązany. Nie ma wtedy potrzeby
rozbijania go na podproblemy.

Jeżeli bezpośredniego dowodu nie uda się znaleźć w przyjętych limitach, system
próbuje wygenerować jedno lub kilka rozbić tego sekwentu. Poprawne rozbicia są
dodawane jako dzieci rozwijanego wierzchołka, a do każdego z nich dodawane są
następnie dzieci odpowiadające podproblemom danego rozbicia.

Schemat rozwijania liścia można zapisać tak:
\[
\begin{array}{l}
\textsc{RozwińLiść}(LP):\\[0.4ex]
\quad \text{spróbuj znaleźć bezpośredni dowód sekwentu } LP,\\
\quad \text{jeżeli dowód został znaleziony, oznacz } LP \text{ jako rozwiązany,}\\
\quad \text{w przeciwnym przypadku wygeneruj poprawne rozbicia } LP,\\
\quad \text{dodaj każde rozbicie jako dziecko typu } SP,\\
\quad \text{dla każdego rozbicia dodaj jego podproblemy jako dzieci typu } LP,\\
\quad \text{zaktualizuj wartości logiczne odpowiednich wierzchołków.}
\end{array}
\]

W ten sposób system łączy dwa tryby działania. Dopóki możliwe jest znalezienie
dowodu bezpośrednio, korzysta z generowania liniowego. Dopiero gdy to się nie uda,
przechodzi do bardziej strukturalnego kroku, czyli dekompozycji celu.

\subsection{Rekurencyjne przeszukiwanie drzewa}

Ostateczna strategia polega na rekurencyjnym przeszukiwaniu opisanego drzewa.
Jeżeli aktualny wierzchołek jest już oznaczony jako rozwiązany, procedura kończy się
sukcesem. Jeżeli jest nierozwiązanym liściem typu \(LP\), system próbuje go rozwinąć.
Jeżeli jest wewnętrznym wierzchołkiem typu \(LP\), wystarczy znaleźć rozwiązanie dla
jednego z jego dzieci. Jeżeli natomiast jest wierzchołkiem typu \(SP\), trzeba znaleźć
rozwiązania dla wszystkich jego dzieci.

Pseudokod można zapisać następująco:
\[
\begin{array}{l}
\textsc{Przeszukaj}(v):\\[0.4ex]
\quad \textbf{jeżeli } Value(v)=True:\\
\qquad \textbf{zwróć } True,\\[0.4ex]

\quad \textbf{jeżeli } v \text{ jest liściem typu } LP:\\
\qquad \textsc{RozwińLiść}(v),\\
\qquad \textbf{zwróć } Value(v),\\[0.4ex]

\quad \textbf{jeżeli } v \text{ jest wewnętrznym wierzchołkiem typu } LP:\\
\qquad \text{wybierz jedno z jego dzieci typu } SP,\\
\qquad \textbf{jeżeli } \textsc{Przeszukaj}(SP)=True:\\
\qquad\qquad \text{zaktualizuj wartość } v,\\
\qquad \textbf{zwróć } Value(v),\\[0.4ex]

\quad \textbf{jeżeli } v \text{ jest wierzchołkiem typu } SP:\\
\qquad \textbf{dla każdego dziecka } u \text{ wierzchołka } v:\\
\qquad\qquad \textbf{jeżeli } \textsc{Przeszukaj}(u)=False:\\
\qquad\qquad\qquad \textbf{zwróć } False,\\
\qquad \text{zaktualizuj wartość } v,\\
\qquad \textbf{zwróć } True.
\end{array}
\]

Ta procedura odzwierciedla różnicę między dwoma typami wierzchołków. W przypadku
wierzchołka typu \(LP\) wystarczy znaleźć jedną skuteczną drogę dowodu, dlatego
wybór między rozbiciami ma charakter alternatywny. W przypadku wierzchołka typu
\(SP\) dane rozbicie narzuca listę podproblemów, z których każdy musi zostać
udowodniony, dlatego przeszukiwanie ma charakter koniunkcyjny.

Strategia trzecia jest więc połączeniem wcześniejszych metod z planowaniem
struktury dowodu. Model językowy jest używany zarówno do generowania dowodów
sekwentów, jak i do proponowania rozbić celów na podproblemy, natomiast formalny
weryfikator sprawdza poprawność wszystkich wygenerowanych kroków. Dzięki temu
system może wykorzystywać elastyczność modelu językowego, ale nie rezygnuje
z formalnej kontroli poprawności.
...
dowody tych podproblemów w jeden dowód problemu wyjściowego.
