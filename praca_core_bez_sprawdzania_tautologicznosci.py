    ...
    while True:
        ...
        if   satGamma1 & satphi<=satpsi:
            if satGamma2 & satpsi<= satphi:
                sub_problem_Left = LittleProblem(Context(list(set(Gamma1 + [phi])), []), psi)
                sub_problem_Right = LittleProblem(Context(list(set(Gamma2 + [psi])), []), phi)
                subproblems = [sub_problem_Left, sub_problem_Right]
                Gamma1 = list(set(Gamma1))
                Gamma2 = list(set(Gamma2))
                if to_infix(phi) == to_infix(psi):
                    if phi != psi:
                        print("TUU JEST BŁĄÐ")
                if phi in Gamma1:
                    Gamma1.remove(phi)
                if psi in Gamma2:
                    Gamma2.remove(psi)
                problem = LittleProblem(Context([], Gamma1 + Gamma2), Iff(phi, psi))
                rule = IffIntroduction()
                ans = smashed_problem(problem, subproblems, rule)
                is_smashed_correctly(ans)
                check_smash_from_text(pretty_print_smashed_problem(ans))
                return ans
