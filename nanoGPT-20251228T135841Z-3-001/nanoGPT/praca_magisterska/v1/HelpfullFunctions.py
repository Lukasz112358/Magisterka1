...
from praca_magisterska.v1 import TermAndFormulas
from praca_magisterska.v1 import ContextsAndLP


def FreeVariables(Expr):
    if not (isinstance(Expr, TermAndFormulas.Term) or isinstance(Expr, TermAndFormulas.Formula) or isinstance(Expr,
                                                                                                              ContextsAndLP.Context)):
        raise TypeError("Bad arguments")
    if isinstance(Expr, TermAndFormulas.Variable):
        return {Expr}
    if isinstance(Expr, TermAndFormulas.Function_with_argument):
        ans = set()
        for i in Expr.Arguments:
            ans = ans.union(FreeVariables(i))
        return ans
    if isinstance(Expr, TermAndFormulas.Atom):
        ans = set()
        for i in Expr.Arguments:
            ans =ans.union(FreeVariables(i))
        return ans
    if isinstance(Expr, TermAndFormulas.Truth) or isinstance(Expr, TermAndFormulas.Lie):
        return set()
    if isinstance(Expr, TermAndFormulas.Negation) or isinstance(Expr, TermAndFormulas.Conjunction) or isinstance(Expr,
                                                                                                                 TermAndFormulas.Disjunction) or isinstance(Expr,
                                                                                                                                                            TermAndFormulas.Implication) or isinstance(Expr,
                                                                                                                                                                                                       TermAndFormulas.Iff) or isinstance(Expr,
                                                                                                                                                                                                                                          TermAndFormulas.Equal):
        ans = set()
        for i in Expr.Interior:
            ans = ans.union(FreeVariables(i))
        return ans
    if isinstance(Expr, TermAndFormulas.Forall) or isinstance(Expr, TermAndFormulas.Exists):
...
def Substitution(t,x,Expr,formulas_allowed = False):
    if formulas_allowed:
        if not (isinstance(t, TermAndFormulas.Term) or isinstance(t, TermAndFormulas.Formula)):
            raise TypeError("Bad arguments")
    elif not isinstance(t, TermAndFormulas.Term):
        raise TypeError("Bad arguments")
    if not isinstance(x, TermAndFormulas.Variable):
        raise TypeError("Bad arguments")
    if not (isinstance(Expr, TermAndFormulas.Term) or isinstance(Expr, TermAndFormulas.Formula)):
        raise TypeError("Bad arguments")
    if isinstance(Expr, TermAndFormulas.Variable) or isinstance(Expr, TermAndFormulas.Truth) or isinstance(Expr,
                                                                                                           TermAndFormulas.Lie):
        if Expr == x:
            return t
        else:
    ...
    if isinstance(Expr, TermAndFormulas.Function_with_argument):
        ...
        ans = TermAndFormulas.Function_with_argument(Expr.Function, *args)
        return ans
    if isinstance(Expr, TermAndFormulas.Atom):
        args = []
        for i in Expr.Arguments:
            args.append(Substitution(t,x,i,formulas_allowed=formulas_allowed))
        ans = TermAndFormulas.Atom(Expr.Relation, *args)
        return ans
    ...
    if isinstance(Expr, TermAndFormulas.Negation):
        ...
        return TermAndFormulas.Negation(*ans_interior)

    if isinstance(Expr, TermAndFormulas.Equal):
        ans_interior = []
        for i in Expr.Interior:
            ans_interior.append(Substitution(t,x,i,formulas_allowed=formulas_allowed))
        return TermAndFormulas.Equal(*ans_interior)

    if isinstance(Expr, TermAndFormulas.Conjunction):
        ans_interior = []
        for i in Expr.Interior:
            ans_interior.append(Substitution(t,x,i,formulas_allowed=formulas_allowed))
        return TermAndFormulas.Conjunction(*ans_interior)

    if isinstance(Expr, TermAndFormulas.Disjunction):
        ans_interior = []
        for i in Expr.Interior:
            ans_interior.append(Substitution(t,x,i,formulas_allowed=formulas_allowed))
        return TermAndFormulas.Disjunction(*ans_interior)

    if isinstance(Expr, TermAndFormulas.Implication):
        ans_interior = []
        for i in Expr.Interior:
            ans_interior.append(Substitution(t,x,i,formulas_allowed=formulas_allowed))
        return TermAndFormulas.Implication(*ans_interior)

    if isinstance(Expr, TermAndFormulas.Iff):
        ans_interior = []
        for i in Expr.Interior:
            ans_interior.append(Substitution(t,x,i,formulas_allowed=formulas_allowed))
        return TermAndFormulas.Iff(*ans_interior)

    if isinstance(Expr, TermAndFormulas.Forall): # dla uproszczenia załóżmy, że Forall wiąże tylko jedną zmienną
        y = Expr.Vars[0]
        phi = Expr.Interior[0]
        if x  == y:
            return Expr
        if (x != y) and ((not y in FreeVariables(t)) or (not x in FreeVariables(phi))):
            ...
            return TermAndFormulas.Forall(y, *ans_interior)

    if isinstance(Expr, TermAndFormulas.Exists): # dla uproszczenia załóżmy, że Forall wiąże tylko jedną zmienną
        y = Expr.Vars[0]
        phi = Expr.Interior[0]
        if x  == y:
            return Expr
        if (x != y) and ((not y in FreeVariables(t)) or (not x in FreeVariables(phi))):
            ans_interior = [Substitution(t,x,phi,formulas_allowed=formulas_allowed)]
            return TermAndFormulas.Exists(y, *ans_interior)
        else:
            raise TypeError("Bad arguments")
